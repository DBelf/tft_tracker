...
@Service
class TftMatchService(private val summonerService: SummonerService,
                      @Qualifier("riotRegionRestTemplate") private val restTemplate: RestTemplate) {
    ...
    private fun getMatchHistoryIds(summoner: SummonerName) =
            try {
                summonerService.getSummonerByName(summoner)
                        .puuid
                        .let { restTemplate.getForEntity<List<String>>(MATCHES_BY_SUMMONER_ID_ENDPOINT, it, MAX_NUMBER_OF_MATCHES_TO_FETCH) }
                        .body
                        ?.map { MatchId(it) }
    ...
    data class ParticipantDTO(@JsonProperty val placement: Int, @JsonProperty val lastRound: Int, @JsonProperty val puuid: String, @JsonProperty val totalDamageToPlayers: Int)
